...
/**
 * StatView class holds the title, author, and count for each type of state in the Simulation The
 * class holds two HBoxes: one for the general info (title + author) and another for the stats Used
 * in SimulationView and CAController One StatBox is created for each added Simulation
 *
 * @author Eric Doppelt
 */
public class StatView {
  ...
  private static final BackgroundFill STAT_BACKGROUND = new BackgroundFill(BACKGROUND_COLOR,
  ...
  /**
   * Basic constructor takes in the title, author, and stats of the simulation to display Calls
   * renderStatBox to create the VBox
   *
   * @param title  is the title of the simulation
   * @param author is the author of the simulation
   * @param stats  is a Map connecting each state to its current count in the instance of the
   *               simulation
   */
  ...
  /**
   * Updates the StatBox by wiping the current stats and replacing them with the new counts for each
   * state Called each time a grid updates itself to reflect the new instance
   *
